...
export default function PowerpointToPdfTool() {
  ...
  const processFile = (file: File) => {
    ...
    if (!validTypes.includes(file.type)) {
      toast({ title: 'Invalid File Type', description: 'Please upload a PowerPoint file (.ppt, .pptx).', variant: 'destructive' });
      return;
    }
    setError(null);
    ...
    reader.onload = async (e) => {
      ...
      try {
        const result = await powerpointToPdf({ powerpointDataUri: dataUri });
  ...
  const guideProps = {
    ...
    steps: [
      { title: "Upload PowerPoint", description: "Select or drag and drop your .pptx or .ppt file." },
      { title: "Automatic Conversion", description: "The tool automatically converts your presentation into a PDF document." },
          ...
          {!originalFile ? (
            <motion.div key="upload" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              >
                <div className="flex flex-col items-center justify-center text-primary">
                  ...
                  <Button size="lg" className="btn-gradient text-white font-bold text-lg px-8 py-6">Choose PowerPoint file</Button>
                  <p className="mt-4 text-muted-foreground">or drop file here</p>
                </div>
              </div>
              <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept=".ppt,.pptx,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation" />
            </motion.div>
          ) : (
            <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center p-12 bg-card rounded-2xl"><CheckCircle className="h-16 w-16 text-green-500 mb-4" /><p className="text-2xl font-bold">Conversion Complete!</p><p className="font-semibold mt-4 text-lg bg-primary/10 text-primary px-4 py-2 rounded-lg">{originalFile.name.replace(/\.pptx?$/i, '.pdf')}</p><div className="mt-8 flex flex-wrap justify-center gap-4"><Button size="lg" onClick={handleDownload} className="btn-gradient text-white"><Download className="mr-2" />Download PDF</Button><Button size="lg" onClick={handleReset} variant="outline"><X className="mr-2" />Convert Another</Button></div></div>
              )}
            </motion.div>
          )}
        </AnimatePresence>
