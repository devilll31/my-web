...
export default function PdfToExcelTool() {
  ...
  const guideProps = {
    ...
    steps: [
      { title: "Upload PDF", description: "Select or drag and drop your PDF file containing tables." },
      { title: "Automatic Conversion", description: "Our AI will process the file, identify tables, and convert them into an Excel spreadsheet." },
      { title: "Download Excel File", description: "Once finished, download your new, editable .xlsx file." }
    ],
    features: [
      { icon: FileSpreadsheet, title: "Editable Data", description: "Turn static PDF tables into fully editable Excel spreadsheets for analysis and reporting." }
    ]
  };
          ...
          {!originalFile ? (
            ...
            <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center p-12 bg-card rounded-2xl"><CheckCircle className="h-16 w-16 text-green-500 mb-4" /><p className="text-2xl font-bold">Conversion Complete!</p><p className="font-semibold mt-4 text-lg bg-primary/10 text-primary px-4 py-2 rounded-lg">{originalFile.name.replace(/\.pdf$/i, '.xlsx')}</p><div className="mt-8 flex flex-wrap justify-center gap-4"><Button size="lg" onClick={handleDownload} className="btn-gradient text-white"><Download className="mr-2" />Download XLSX</Button><Button size="lg" onClick={handleReset} variant="outline"><X className="mr-2" />Convert Another</Button></div></div>
              )}
            </motion.div>
          )}
        </AnimatePresence>
