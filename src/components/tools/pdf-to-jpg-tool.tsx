...
import { pdfToJpg } from '@/ai/flows/pdf-to-jpg';
...
export default function PdfToJpgTool() {
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        const result = await pdfToJpg({ pdfDataUri: dataUri });
        const dataUriResult = 'data:image/jpeg;base64,' + result.base64Data;
  ...
  const guideProps = {
    ...
    steps: [
      ...
      { title: "Automatic Conversion", description: "The tool automatically converts the first page of your PDF into a high-quality JPG image." },
      { title: "Download JPG", description: "Download your new JPG file, ready to be used." }
    ],
    features: [
      { icon: FileImage, title: "High-Quality Conversion", description: "Get a high-resolution JPG image from your PDF document, perfect for previews or sharing." }
    ]
  };
          ...
          {!originalFile ? (
            ...
            <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center p-12 bg-card rounded-2xl"><CheckCircle className="h-16 w-16 text-green-500 mb-4" /><p className="text-2xl font-bold">Conversion Complete!</p><p className="font-semibold mt-4 text-lg bg-primary/10 text-primary px-4 py-2 rounded-lg">{originalFile.name.replace(/\.pdf$/i, '.jpg')}</p><div className="mt-8 flex flex-wrap justify-center gap-4"><Button size="lg" onClick={handleDownload} className="btn-gradient text-white"><Download className="mr-2" />Download JPG</Button><Button size="lg" onClick={handleReset} variant="outline"><X className="mr-2" />Convert Another</Button></div></div>
              )}
            </motion.div>
          )}
        </AnimatePresence>
