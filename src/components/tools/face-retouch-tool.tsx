...
import { faceRetouch } from '@/ai/flows/face-retouch';
import { AnimatePresence, motion } from 'framer-motion';

export default function FaceRetouchTool() {
  ...
  const handleRetouch = async () => {
    ...
    setIsLoading(true);
    try {
      const result = await faceRetouch({ photoDataUri: originalImage });
      setProcessedImage(result.retouchedPhotoDataUri);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred';
      setError(errorMessage);
      toast({
        title: 'Error Retouching Image',
  ...
  const handleDownload = () => {
    if (processedImage) {
      ...
      link.download = 'face-retouched-image.png';
    ...
    <div className="w-full">
      ...
      <AnimatePresence>
        {originalImage && (
          ...
          >
            ...
            <div>
              <h3 className="text-center font-semibold mb-2">Retouched Image</h3>
              <div className="relative aspect-square w-full rounded-lg overflow-hidden border bg-[url('/grid.svg')]">
                {isLoading && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-10">
                    <div className="text-center text-white">
                      <p className="font-bold">Retouching...</p>
                      <p className="text-sm">AI is enhancing the faces.</p>
      ...
      {originalImage && (
        <div className="mt-6 flex flex-wrap justify-center gap-4">
          ...
          {!processedImage && (
            <Button onClick={handleRetouch} disabled={isLoading}>
              <Wand2 className="mr-2" />
              {isLoading ? 'Retouching...' : 'Retouch Faces'}
            </Button>
          )}
