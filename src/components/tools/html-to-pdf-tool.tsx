...
import { htmlToPdf } from '@/ai/flows/html-to-pdf';
...
export default function HtmlToPdfTool() {
  ...
  const guideProps = {
    ...
    steps: [
      { title: "Upload HTML File", description: "Select or drag and drop your .html file." },
      { title: "Automatic Conversion", description: "The tool will automatically render the HTML and convert it into a PDF document." },
      { title: "Download PDF", description: "Download your new PDF, preserving the layout of your webpage." }
    ],
    features: [
      { icon: FileText, title: "Webpage to Document", description: "Create a shareable, print-ready PDF from any HTML file, perfect for archiving or offline viewing." }
    ]
  };
          ...
          {!originalFile ? (
            <motion.div key="upload" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              >
                <div className="flex flex-col items-center justify-center text-primary">
                  ...
                  <Button size="lg" className="btn-gradient text-white font-bold text-lg px-8 py-6">Choose HTML file</Button>
                  <p className="mt-4 text-muted-foreground">or drop file here</p>
                </div>
              </div>
              <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept=".html,.htm" />
            </motion.div>
          ) : (
            <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center p-12 bg-card rounded-2xl"><CheckCircle className="h-16 w-16 text-green-500 mb-4" /><p className="text-2xl font-bold">Conversion Complete!</p><p className="font-semibold mt-4 text-lg bg-primary/10 text-primary px-4 py-2 rounded-lg">{originalFile.name.replace(/\.html?$/i, '.pdf')}</p><div className="mt-8 flex flex-wrap justify-center gap-4"><Button size="lg" onClick={handleDownload} className="btn-gradient text-white"><Download className="mr-2" />Download PDF</Button><Button size="lg" onClick={handleReset} variant="outline"><X className="mr-2" />Convert Another</Button></div></div>
              )}
            </motion.div>
          )}
        </AnimatePresence>
