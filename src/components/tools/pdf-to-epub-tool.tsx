...
import { pdfToEpub } from '@/ai/flows/pdf-to-epub';
...
export default function PdfToEpubTool() {
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        const result = await pdfToEpub({ pdfDataUri: dataUri });
        const dataUriResult = 'data:application/epub+zip;base64,' + result.base64Data;
  ...
  const guideProps = {
    ...
    steps: [
      ...
      { title: "Automatic Conversion", description: "The tool automatically converts your PDF into a reflowable EPUB format." },
      { title: "Download EPUB", description: "Download your new eBook, ready for any e-reader." }
    ],
    features: [
      { icon: FileText, title: "eReader Ready", description: "Convert your PDFs into the standard EPUB format for a better reading experience on devices like Kindle, Kobo, and more." }
    ]
  };
          ...
          {!originalFile ? (
            ...
            <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center p-12 bg-card rounded-2xl"><CheckCircle className="h-16 w-16 text-green-500 mb-4" /><p className="text-2xl font-bold">Conversion Complete!</p><p className="font-semibold mt-4 text-lg bg-primary/10 text-primary px-4 py-2 rounded-lg">{originalFile.name.replace(/\.pdf$/i, '.epub')}</p><div className="mt-8 flex flex-wrap justify-center gap-4"><Button size="lg" onClick={handleDownload} className="btn-gradient text-white"><Download className="mr-2" />Download EPUB</Button><Button size="lg" onClick={handleReset} variant="outline"><X className="mr-2" />Convert Another</Button></div></div>
              )}
            </motion.div>
          )}
        </AnimatePresence>
